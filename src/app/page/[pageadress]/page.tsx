                                    ...
                                    <Col>
                                        <Tab.Content className={cx("user-data")}>
                                            ...
                                            <Tab.Pane eventKey="blogs">
                                                {data?.info?.nick_name && (
                                                    <DataShowBlogs
                                                        isReload_={isReload_Blogs}
                                                        setIsReload_={setIsReload_Blogs}
                                                        userpage={data.info.nick_name}
                                                    />
                                                )}
                                            </Tab.Pane>
...
function DataShowBlogs({ userpage, isReload_, setIsReload_ }: PropsDataShowBlogs) {
    const [{ data, loading, error }, refetch] = useAxios<Blog[]>({
        baseURL: ServerPath,
        url: `/blogs/user/${userpage}`,
    });
                        ...
                        data.map((blog, index) => (
                            <div key={index} className={cx("blog")}>
                                <div className={cx("header")}>
                                    <Link href={`/${blog?.owner?.page_url}`}>
                                        <Image src={blog?.owner?.avatar?.[0]?.url} alt="avatar" className={cx("header__avatar")} />
                                    </Link>
                                    <div className={cx("header__info")}>
                                        <span className={cx("header__info-name")}>{blog?.owner?.name}</span>
